...
public class JsonCacheUtil {
    ...
    public void saveCache(String key, Object value, long duration) {
        ...
        //保存一个有数据有时间的内容
        CacheWidthDuration cacheWidthDuration = new CacheWidthDuration(duration, valueStr);
        String cacheWithTime = mGson.toJson(cacheWidthDuration);
    ...
    public <T> T getValue(String key, Class<T> clazz) {
        ...
        CacheWidthDuration cacheWidthDuration = mGson.fromJson(valueWithDuration, CacheWidthDuration.class);
        //对时间进行判断
        long duration = cacheWidthDuration.getDuration();
        if (duration != -1 && duration - System.currentTimeMillis() < 0) {
            ...
            //没过期
            String cache = cacheWidthDuration.getCache();
